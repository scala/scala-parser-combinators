...
lazy val parserCombinators = crossProject(JVMPlatform, JSPlatform, NativePlatform)
  ...
  .nativeSettings(
    ...
    Test / fork := false,
    libraryDependencies :=
        libraryDependencies.value.filterNot(_.organization == "junit") :+ "org.scala-native" %%% "junit-runtime" % "0.4.8",
    addCompilerPlugin("org.scala-native" % "junit-plugin" % "0.4.8" cross CrossVersion.full)
  )
